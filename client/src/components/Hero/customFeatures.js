...
function isCustomSectionHeading(line) {
...
function isExplicitCustomEnd(line) {
...
function splitAbilitiesText(text) {
...
function normalizeCustomFeature(input, index) {
...
function parseCustomTextSection(text) {
  if (!text) return [];
...
export function getCharacterCustomFeatures(character) {
  const stored = Array.isArray(character?.custom_features)
    ? character.custom_features
      .map((item, index) => normalizeCustomFeature(item, index))
      .filter(Boolean)
    : [];
...
export function getCharacterNotesText(text) {
